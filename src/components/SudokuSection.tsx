...
import {
  SudokuSectionCoord,
  getIndexiesInSection,
  getSudokuBoardCoordFromIndex,
  getSudokuSectionCoordFromIndex,
} from "../util/Util";
...
export default function SudokuSection({
  ...
  function getIsHighlighted(index: number, focussedIndex: number) {
    ...
    const sameRowOrCol =
      assocBoardCoord.row == focussedBoardCoord.row ||
      assocBoardCoord.col == focussedBoardCoord.col;
    const sameSection =
      assocSection.row == focussedSection.row &&
      assocSection.col == focussedSection.col;
    return sameSection || sameRowOrCol;
  }

  let sectionIndecies = getIndexiesInSection(sectionCoord);
  let sectionSolved =
    sectionIndecies.filter((index) => {
      return populatedBoard[index] != solvedBoard[index];
    }).length == 0;
