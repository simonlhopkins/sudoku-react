...
var randomSeed = seedRandom("simon")
function makepuzzle(board, seed) {
    randomSeed = seedRandom(seed);
    var puzzle = [];
  ...
  function randomInt(max) {
    return Math.floor(randomSeed() * (max + 1));
  }
  ...
  function shuffleArray(original) {
    ...
    for (var i = 0; i < original.length; i++) {
      ...
      while (j === i) {
        j = Math.floor(randomSeed() * original.length);
      }
...
export {makepuzzle, solvepuzzle, ratepuzzle, posfor}